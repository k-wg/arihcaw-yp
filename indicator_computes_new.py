...
class WebSocketMarketData:
    ...
    def should_refresh_0300_cache(self):
        ...
        # Refresh if it's a new day after 03:00
        if current_date > self.last_cache_update_date and kenyan_time.hour >= 3:
            return True
            ...
            for token in TOKEN_WATCHLIST.keys():
                try:
                    ...
                    for pairing in TOKEN_WATCHLIST[token]["pairings"]:
                        symbol = f"{token}{pairing}"
                        try:
                            price_0300 = get_price_at_kenyan_0300(client, symbol)
                    ...
                    if price_0300 is not None:
                        ...
                        logger.info(
                            f"✅ Successfully cached 03:00 price for {token}{used_pairing}: {price_0300}"
                        )
                    else:
                        logger.warning(
...
def get_price_at_kenyan_0300(client, symbol):
    """
    Get the price at 03:00 Kenyan time (00:00 UTC) for any symbol
    """
    try:
        ...
        # Set target time to 03:00 Kenyan time (which is 00:00 UTC)
        target_kenyan_time = kenyan_time.replace(
            hour=3, minute=0, second=0, microsecond=0
        )

        # If current time is before 03:00, use yesterday's 03:00
        ...
        # Get the kline for 03:00 Kenyan time
        ...
        if klines:
            return float(klines[0][4])  # Return the close price
        else:
            return None

    except Exception as e:
        logger.debug(f"Error getting price at 03:00 for {symbol}: {e}")
        return None
    ...
    try:
        ...
        with token_price_lock:
            if "SOL" in market_data_manager.token_data:
                current_price = market_data_manager.token_data["SOL"]["current_price"]
            else:
...
class StatefulIndicatorEngine:
    ...
    def initialize_from_history(self, historical_df):
        ...
        daily_diff = get_daily_difference_kenyan_time()
        self.current_df["daily_diff"] = daily_diff
...
def create_market_overview_panel(token_data):
    ...
    token_cells = []
    for token in TOKEN_WATCHLIST.keys():
        if token in token_data and token_data[token]["status"] == "OK":
            ...
            # Determine price color based on value (you can customize this logic)
            price_color = (
                "cyan"
            ...
            status = (
                token_data.get(token, {}).get("status", "N/A")
    ...
    if token_data:
        positive_tokens = sum(
            ...
            if data.get("status") == "OK" and data["change"] > 0
        )
        total_tokens = len(
            [data for data in token_data.values() if data.get("status") == "OK"]
        )
        sentiment = (
            f"Market Sentiment: {positive_tokens}/{total_tokens} tokens positive"
        )

        # Add sentiment to title
        title = f"🔄 MARKET OVERVIEW • {sentiment} • 03:00 EAT Base • 📡 WebSocket"
    else:
        title = "🔄 MARKET OVERVIEW • 03:00 EAT Base • 📡 WebSocket"
...
def continuous_indicator_calculator():
    ...
    # Initialize stateful engine
    stateful_engine = StatefulIndicatorEngine()
    ...
    startup_info.add_row(
        f"📡 WebSocket: {'✅ Connected' if websocket_started else '❌ Failed'}"
    )
    startup_info.add_row("")
        ...
        while True:
            ...
            if is_updated or not engine_initialized:
                ...
                if update_count == 0 or (time.time() % 30 < UPDATE_INTERVAL):
                    ...
                    console.print(
                        f"[yellow]⏳ {status} (Update #{update_count}) | WebSocket: {ws_status} | Running: {running_time}[/yellow]"
                    )
...
"""
    ...
    if choice == "1":
        ...
        console.print(
            Panel(
                "🔄 Starting Continuous Mode with WebSocket Data...",
                style="bold cyan",
                box=box.DOUBLE,
            )
        )
